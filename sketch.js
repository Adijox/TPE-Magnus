...
txt[3] = createDiv('Vitesse angulaire donnée à la balle : ' + s + ' tours/s ( ' + Number((s)*(2*PI)).toFixed(1) + ' tours/sec)').position(880, 200);
...
// on calcule d'abord la vitesse initiale de la balle sur les y
    ...
    var ay = -g + ((1/2)*(rho*PI*r*r*viy*viy * (-sin(angle)*ct))
                  +(4/3)*4*PI*PI*r*r*r*-s*rho*viy*cos(angle))/m;
...
//on en déduit la nouvelle position sur y
    ...
    posy[step] = yf;
            ...
            accy[i] = -g + ((1/2)*(rho*PI*r*r*velsy[i]*velsy[i]*(-sin(angle)*ct))
                           +(4/3)*4*PI*PI*r*r*r*-s*rho*velsy[i]*cos(angle))/m;  
            }
        }
    }
